import { ImportRuleRepository, ImportRuleFilters } from "../domain/repositories/ImportRuleRepository";
...
export default class ImportRuleD2ApiRepository implements ImportRuleRepository {
    ...
    async getAll(filters?: ImportRuleFilters): Promise<ImportRule[]> {
        const importRulesData = await this.dataStore.get<ImportRuleData[]>(this.dataStoreKey).getData();
            ...
            const importRules = importRulesData?.map(importRuleData => {
                return {
                    ...
                    periodInformation: {
                        ...
                        startDate: importRuleData.periodInformation.startDate ? new Date(importRuleData.periodInformation.startDate) : undefined,
                        endDate: importRuleData.periodInformation.endDate ? new Date(importRuleData.periodInformation.endDate) : undefined,
                    }
                };
            });
            return importRules;
