...
export class Either<Error, Data> {
    ...
    isError(): this is { value: EitherValueError<Error> } {
        return this.value.type === "error";
    }

    isSuccess(): this is { value: EitherValueSuccess<Data> } {
    ...
    fold<T>(error: (left: Error) => T, success: (right: Data) => T): T {
        return this.match({ error, success });
    }
