...
export class DeleteImportRulesUseCase {
    ...
    private async updateRelatedImportSummaries(
        importRulesIdToDelete: Id[]
        ...
        const editedImportSummaries = importSummariesWithRulesIdTodelete.map(importSummary => {
            const importRule = importRules.find(
                importRule => importRule.id === importSummary.importRule
            );

            return importRule
                ? importSummary.updateWithDeletedImportRule(
                      i18n.t("{{name}} (deleted)", importRule)
                  )
                : importSummary;
        });
