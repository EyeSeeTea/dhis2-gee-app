...
describe("ImportUseCase", () => {
    it("should import expected data value set and return expected message", async () => {
        ...
        const importUseCase = new ImportUseCase(
            geeDataSetRepository, geeDataRepository, orgUnitRepository, dataValueSetRepository);

        const importRule = givenAImportRule();

        const result = await importUseCase.execute(importRule);
        ...
        expect(result).toEqual({
            failures: [],
            messages: ["6 data values from ERA5 - DAILY google data set.", "Imported: 6 - updated: 0 - ignored: 0"],
            success: true
        });
    });
});
...
function givenAImportRule(): ImportRule {
    return {
        id: "Default",
        name: "Default",
        ...
        selectedMappings: [{
            ...
            "/E4h5WBOg71F/RnDSNvg7zR2/KwLlNNoREes/GY1mRd2suuj/WFAboRxdVjA"
        ]
    }
}
...
function givenAOrgUnitRepository(): OrgUnitRepository {
    return {
        getByIds: jest.fn().mockImplementation(() => {
            return [{ "id": "WFAboRxdVjA", "coordinates": "[-2.708309,6.675618]", "featureType": "POINT" }]
        })
    }
}
